...
export default function Home() {
  ...
  return (
    <div style={{ backgroundColor: '#0d0d0d', color: '#fff', minHeight: '100vh', fontFamily: 'sans-serif' }}>
      ...
      {/* ======= 2 SECTION TABS ======= */}
      <div style={{ display: 'flex', justifyContent: 'center', gap: '12px', padding: '15px' }}>
        <button onClick={() => setTab('audio')} style={{ padding: '12px 28px', backgroundColor: tab === 'audio' ? '#ff0000' : '#1a1a1a', color: 'white', border: '1px solid ' + (tab === 'audio' ? '#ff0000' : '#333'), borderRadius: '25px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' }}>🎧 Sunne Wali</button>
        <button onClick={() => setTab('text')} style={{ padding: '12px 28px', backgroundColor: tab === 'text' ? '#ff0000' : '#1a1a1a', color: 'white', border: '1px solid ' + (tab === 'text' ? '#ff0000' : '#333'), borderRadius: '25px', cursor: 'pointer', fontSize: '1rem', fontWeight: 'bold' }}>📖 Padhne Wali</button>
      </div>
      ...
      <div style={{ maxWidth: '800px', margin: '0 auto', padding: '15px' }}>
        ...
        {isAdmin && (
          <div style={{ backgroundColor: '#161616', padding: '20px', borderRadius: '12px', marginBottom: '25px', border: '1px solid #ff0000' }}>
            ...
            <p style={{ color: '#888', fontSize: '0.85rem', marginTop: 0 }}>Sirf Audio dalo toh 🎧 section mein aayegi. Sirf Text likho toh 📖 mein. Dono dalo toh dono mein!</p>
        ...
        {!loading && showList.length === 0 && <p style={{ color: '#666', textAlign: 'center', padding: '40px' }}>Is section mein abhi kuch nahi hai...</p>}
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '15px' }}>
          {showList.map((story) => (
            <div key={story.id} onClick={() => setReadingStory(story)} style={{ backgroundColor: '#161616', borderRadius: '12px', overflow: 'hidden', border: '1px solid #2a2a2a', cursor: 'pointer', position: 'relative' }}>
              ...
              {story.audio && <span style={{ position: 'absolute', top: '8px', right: '8px', backgroundColor: 'rgba(255,0,0,0.85)', borderRadius: '20px', padding: '3px 8px', fontSize: '0.7rem' }}>🎧</span>}
              {story.price > 0 && <span style={{ position: 'absolute', top: '8px', left: story.audio ? '8px' : 'auto', right: story.audio ? 'auto' : '8px', backgroundColor: 'rgba(255,180,0,0.9)', color: '#000', borderRadius: '20px', padding: '3px 8px', fontSize: '0.7rem', fontWeight: 'bold' }}>🔒 ₹{story.price}</span>}
              {isAdmin && (
